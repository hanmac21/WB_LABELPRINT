...
function bindEvents() {
	...
	// 엔터키 검색
	$("#itemtype, #car, #itemcode, #spec, #itemname, #custname").on(
		"keydown",
		function (e) {
			if (e.key === "Enter") {
				search();
	...
	$("#supplierToggle").on("click", function () {
		$("#supplierDropdown").toggleClass("active");
	});

	$("#supplier").on("focus", function () {
		$("#supplierDropdown").addClass("active");
	});

	$(document).on("click", function (e) {
		if (!$(e.target).closest(".supplier-combo").length) {
			$("#supplierDropdown").removeClass("active");
		}
	});
}
...
function renderTable(items) {
	...
	if (!items || items.length === 0) {
		$tbody.append(
			`<tr><td colspan="${isPT ? 8 : 7}" class="empty-row">No data found</td></tr>`,
		);
		return;
	}
	...
	items.forEach(function (item, index) {
		// 폐기 여부
		const isDeleted =
			item.deldate != null && String(item.deldate).trim() !== "";

		const cnamePT = isPT
			? `<td class="col-cname">${escapeHtml(item.custname)}</td>`
			: "";
...
async function print() {
	...
	// SPEC에 한글 포함 시 중단
	if (
		!printData.labelType.includes("CUST") &&
		/[ㄱ-ㅎㅏ-ㅣ가-힣]/.test(printData.spec)
	) {
		Modal.alert(
			"고객사 품번에 한글이 포함되어 바코드 생성이 불가능합니다.",
		);
		return;
	}
...
function doPrint(printData) {
	...
	$.ajax({
		...
		error: function (xhr, status, error) {
			console.error(
				"Barcode create failed:",
				status,
				error,
				xhr.responseText,
			);
			Modal.alert(
				"Barcode create failed: " + (xhr.responseText || error),
			);
		},
		complete: hideLoading,
	});
...
function openPrintModal(result, printData) {
	...
	items.forEach(function (item) {
		const $btn = $(
			'<button class="print-modal-btn print-label-btn"></button>',
		)
			.text(item.label)
			.on("click", function () {
...
function closePrintModal() {
	...
	const $notPrinted = $("#printModalButtons .print-label-btn").not(
		".printed",
	);
